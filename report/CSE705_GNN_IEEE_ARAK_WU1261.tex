...
to different things. Both are captured three times per run, before
training, at the selected checkpoint, and at the final epoch, which is
what makes an untrained state available as an explicit reference rather
than an assumption.
...
The proposed remedies act at different points. Residual connections add
an identity path through the layer; PairNorm acts on the geometry of the
representation \citep{zhao2020pairnorm}; Jumping Knowledge changes the
readout instead of the propagation \citep{xu2018representation}; and
GCNII modifies the layer itself \citep{chen2020simple}. Section III
gives each as implemented here.
...
model, because evaluations typically report one collapse metric at one
point in training. This study addresses that gap by holding dataset,
...
with \(\ell_0=\min(\mathcal{B})\); \(E_1\) below denotes that
first-layer energy. The curve is summarized by a contraction slope \(b\)
fitted as
...
GraphSAGE's MAD at depths 16 and 32 sits at the noise level around zero, where
the depth-32 standard deviation exceeds the mean.}
\label{tab:epoch0}
...
across classes. GCNII is the only architecture that ends higher than it
starts, rising from 78.45\% at depth 2 to 83.26\% at depth 32 after a
dip to 75.81\% at depth 4. Its checkpoint contraction slope stays within
roughly \(\pm 0.4\) at every depth (+0.383, +0.169, −0.037, +0.012)
where GCN's runs from +1.64 to +2.46.
