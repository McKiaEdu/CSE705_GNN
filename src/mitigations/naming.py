...
filename (C3), so the naming convention exists in one place.
"""
...
def MitigationNames(layerHooks: Sequence[LayerHook], readout: Readout) -> list[str]:
    ...
    JkReadout. The redundancy with config.readout is accepted per D-028;
    consistency between the two is asserted by experiments/, not here."""
