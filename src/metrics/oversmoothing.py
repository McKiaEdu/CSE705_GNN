...
The measuring instrument: holds the fixed augmented graph as state and turns a
models/ layerEmbeddings list into the per-dimension, per-layer scalars the
study reports.
"""
...
def BuildAugmentedOperator(edgeIndex: Tensor, numNodes: int) -> tuple[Tensor, Tensor]:
    ...
    if contains_self_loops(edgeIndex):
        raise ValueError("edgeIndex must be self-loop-free; OversmoothingMetrics augments internally")
    ...
    # so out-degree on row 0 equals d~_i
...
def SelectComparableBand(layerEmbeddings: Sequence[Tensor]) -> list[int]:
    ...
    Index 0 is excluded unconditionally: it is a different representation kind
    (raw input, not a hidden state) regardless of width. Raises if the
    resulting indices are not contiguous, so an unanticipated architecture
    ...
    # layer 1's hiddenDim, via Counter's insertion-order tie-break, which is
    # the correct band
...
def FitContractionSlope(energies: Sequence[float], bandIndices: Sequence[int]) -> float:
    ...
    # flooring at a small epsilon keeps the fit well-defined instead of raising
    # on log(0), at the cost of capping how negative the fitted slope can read
    # for a fully collapsed run
    ys = torch.tensor(
...
class OversmoothingMetrics:
    ...
    def DirichletEnergy(self, h: Tensor) -> float:
        ...
        divided by h.shape[1]. Self-loop terms contribute exactly 0 to the sum
        but change every d~_i, which is why the graph is augmented at all.
        """
